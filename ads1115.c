...
//	memcpy(&ADS1115_I2C_Handler, handler, sizeof(*handler));
	...
	if (HAL_I2C_IsDeviceReady(ADS1115_I2C_Handler_ptr, (uint16_t) (ADS1115_devAddress << 1), 5, ADS1115_TIMEOUT) == HAL_OK) {
		return HAL_OK;
	} else {
		return HAL_ERROR;
	}
...
HAL_StatusTypeDef ADS1115_readSingleEnded(uint16_t muxPort, float *voltage) {
	...
	if(HAL_I2C_Mem_Write(ADS1115_I2C_Handler_ptr, (uint16_t) (ADS1115_devAddress << 1), ADS1115_CONFIG_REG, 1, ADS1115_config, 2, ADS1115_TIMEOUT) == HAL_OK)
	{
		while(waiting) //Checking Data Ready
		{
			HAL_Delay(1);
			if(HAL_I2C_Mem_Read(ADS1115_I2C_Handler_ptr, (uint16_t) ((ADS1115_devAddress << 1) | 0x1), ADS1115_CONFIG_REG, 1, ADS1115_config, 2, ADS1115_TIMEOUT) == HAL_OK)
				{
				if(ADS1115_config[0] & ADS1115_OS) waiting=0;
				}
			else return HAL_ERROR;
			if(++cnt==250) return HAL_ERROR;
		}

		if(HAL_I2C_Mem_Read(ADS1115_I2C_Handler_ptr, (uint16_t) ((ADS1115_devAddress << 1) | 0x1), ADS1115_CONVER_REG, 1, ADS1115_rawValue, 2, ADS1115_TIMEOUT) == HAL_OK)
		{
